...
async function main() {
  ...
  // 3. REFRESH — since migration 006 the summary tables are materialized in
  //    the source db itself (and copied over by the backup above, indexes
  //    included), so the demo needs no precompute step of its own. Rebuild
  //    them once here anyway so the shipped snapshot is exact even if the
  //    source tables were somehow stale.
  refreshSummaries(db);
